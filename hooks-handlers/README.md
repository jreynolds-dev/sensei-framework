...
This directory documents the original shell script hooks from early versions of Sensei. The scripts have been removed. They were superseded by prompt hooks in `hooks/hooks.json` for security and cross-platform compatibility.
...
**How to verify it ran (historical):** In verbose mode (`Ctrl+O`), the session context would include blocks beginning with `--- SENSEI: TEACHING ENGINE ---` and ending with `--- END SENSEI ---`.
