...
6. **Stage completion celebration:** Compare the before and after stage snapshots from step 5. If any stage went from incomplete to fully complete this session:
...
9. **Ambient learning check** -- If no `/dojo-open` was called this session (no session briefing or micro-goal appears in the conversation history) but significant work was done (new files created, major features added, or new concepts encountered), offer ONE optional learning moment in character voice before closing:
